...
impl Forecast {
    ...
    pub fn new(
        ...
        Self { point_forecast, lower_bound, upper_bound, confidence_level }
    }
...
impl BatchPredictor {
    ...
    fn predict_batch_gpu(
        ...
        // Extract forecasts
        let point_forecasts: Vec<Vec<f64>> = cpu_output.get_item("mean")?.call_method0("tolist")?.extract()?;
        let lower_bounds: Vec<Vec<f64>> = cpu_output.get_item("lower")?.call_method0("tolist")?.extract()?;
        let upper_bounds: Vec<Vec<f64>> = cpu_output.get_item("upper")?.call_method0("tolist")?.extract()?;
...
impl EnsemblePredictor {
    ...
    pub fn predict_ensemble(
        ...
        for (model_name, weight) in self.models.iter().zip(&self.weights) {
            let model = model_dict.get_item(model_name)?
